...
<script type="text/javascript" src="<%=basePath %>/js/sample/storeInfoList.js"></script>
						...
						<tr>
						  ...
						  <c:forEach items="${pagehelper.list}" var="ps"  varStatus="i">
							<tr>
								...
								<td>${ps.store.clothID}</td>
								<td>${ps.store.color}</td>
								<td>${ps.xs}</td>
								<td>${ps.s}</td>
								<td>${ps.m}</td>
								<td>${ps.l}</td>
								...
								<td>${ps.eva}</td>
								<td>${ps.outIn}</td>
								<td><fmt:formatDate value='${ps.date}' pattern='yyyy-MM-dd'/></td>
								<td>${ps.remark}</td>
							</tr>
						  </c:forEach>
					</table>
