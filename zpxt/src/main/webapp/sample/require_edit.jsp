		...
		$("#submitbutton").click(function() {
			 ...
			 for(var i = 0;i<params.length;i++){
			  ...
			  $.ajax({ 
	            ...
	            url: basePath+"/ClothRequireController/update.do", 
...
<input type="text" id="fileName" name="fileName" value="${clothDesignVo.fileName}" hidden="hidden"/>
		...
		<div class="ui_content">
			<table  cellspacing="0" cellpadding="0" width="100%" align="left" border="0">
				<tr>
					...
					<td class="ui_text_lt">
						<input type="text" id="department" value="${clothDesignVo.department}" name="department" class="ui_input_txt02"/><font style="color: red;">*</font>
					</td>
						...
						<select name="requireUserID" id="requireUserID" class="ui_select01">
                            <option value="">--请选择--</option>
                             <c:forEach items="${dList}" var="ps"  varStatus="i">
                                   <option value="${ps.id}" <c:if test="${clothDesignVo.requireUserID == ps.id}">selected</c:if>>${ps.cname}</option>
                             </c:forEach>
					...
					<td class="ui_text_lt">
						<input type="text" id="customerOfficeName" value="${clothDesignVo.customerOfficeName}" name="customerOfficeName" class="ui_input_txt02"/>
					</td>
				...
				<td class="ui_text_rt">数量</td>
					<td class="ui_text_lt">
						<input type="text" id="num" name="num" value="${clothDesignVo.num}" class="ui_input_txt02"/>
					</td>
					<td class="ui_text_rt">预计规还日期</td>
					<td class="ui_text_lt">
						<input id="d11b" name="backDate" type="text" value="<fmt:formatDate value='${clothDesignVo.backDate }' pattern='yyyy-MM-dd'/>" onClick="WdatePicker()"/>
					</td>
					
						<td class="ui_text_rt">用途</td>
					<td class="ui_text_lt">
						<input type="text" id="purpose" name="purpose" value="${clothDesignVo.purpose}"  class="ui_input_txt02"/>
					</td>
				</tr>
				<tr>
						<td class="ui_text_rt">申请理由</td>
					<td class="ui_text_lt">
						<input type="text" id="requestReason" name="requestReason"  value="${clothDesignVo.requestReason}" class="ui_input_txt02"/>
					</td>
						<td class="ui_text_rt">备注</td>
					<td class="ui_text_lt">
						<input type="text" id="remark" name="remark"  value="${clothDesignVo.remark}" class="ui_input_txt02"/></td>
