...
var queryAllPerson = function(pageNum,pageSize) {
	...
	$("#frame-contect", parent.document).attr("src",basePath+"/purchare/getList.do?"+param);
}
...
$(document).ready(function(){
	...
	$("#addBtn").click(function() {
		 $("#frame-contect", parent.document).attr("src",basePath+'/purchare/add.do');
	});
	/** 编辑   **/
	$("#editBtn").click(function() {
		...
		if(num!=1){
		 ...
		 $("#frame-contect", parent.document).attr("src",basePath+'/purchare/edit.do?requireId='+allIDCheck);
	});
...
/** 模糊查询来电用户  **/
function search(){
	$("#submitForm").attr("action", basePath+"/purchare/getList.do?comNo="+$("#comNo").val()).submit();
}
...
function batchDel(){
	...
	if(allIDCheck.length>0) {
		...
		if(confirm("您确定要批量删除这些记录吗？")){
			$("#submitForm").attr("action", basePath+"/purchare/deleteByIds.do?requireIds="+$("#allIDCheck").val()).submit();
		}
	}
}