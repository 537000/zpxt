...
var queryAllPerson = function(pageNum,pageSize) {
	...
	$("#frame-contect", parent.document).attr("src",basePath+"/asset/getList.do?"+param);
}
...
$(document).ready(function(){
	...
	$("#addBtn").click(function() {
		 $("#frame-contect", parent.document).attr("src",basePath+'/asset/add.do');
	});
	/** 编辑   **/
	$("#editBtn").click(function() {
		...
		if(num!=1){
		 ...
		 $("#frame-contect", parent.document).attr("src",basePath+'/asset/edit.do?requireId='+allIDCheck);
	});
...
/** 模糊查询来电用户  **/
function search(){
	$("#submitForm").attr("action", basePath+"/asset/getList.do?word="+$("#word").val()).submit();
}
...
function batchDel(){
	...
	if(allIDCheck.length>0) {
		...
		if(confirm("您确定要批量删除这些记录吗？")){
			$("#submitForm").attr("action", basePath+"/asset/deleteByIds.do?requireIds="+$("#allIDCheck").val()).submit();
		}
	}
}